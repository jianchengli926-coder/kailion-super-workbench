/* ============================================================
   KaiLionCrafts Creator · 真实图像与 PDF 处理 / Image & PDF Tools
   ------------------------------------------------------------
   全部在浏览器内用 Canvas 真实执行（不是推演）：
   · 格式互转 PNG/JPG/WEBP/（读）GIF/BMP
   · 等比缩放、质量压缩、去元数据（重编码天然剥离 EXIF）
   · 九宫格 / 任意行列切割
   · 多图合并为真正的 PDF（自研 PDF 写入器，DCTDecode 嵌入 JPEG）
   ============================================================ */
 ...
 */
export async function convertImage(src, opt = {}) {
 ...
 */
export async function splitGrid(src, rows, cols, opt = {}) {
  const img = await loadImage(src);
 ...
 */
export async function imagesToPdf(images, opt = {}) {
  if (!images?.length) throw new Error('没有可合并的图片');
  const quality = opt.quality !== undefined ? Number(opt.quality) : 0.92;
  ...
  /* --- 文件头：%PDF-1.4 + 二进制标记（缺了它任何阅读器都打不开） --- */
  ...
  // 预分配对象号：每页占 3 个（Page / Image XObject / Contents），从 3 开始
...
export async function downloadZip(files, zipName) {
  ...
  downloadBlob(new Blob([makeZip(prepared)], { type: 'application/zip' }), zipName);
}
...
/** 把 SVG 字符串渲染成 PNG blob（用于把占位图变成真图下载） */
