/* ============================================================
   KaiLionCrafts Creator · OOXML 生成器 / Real Document Export
   ------------------------------------------------------------
...
import { makeZip, bytesOf } from './zip.js';
...
export function buildDocx(blocks, opts = {}) {
  const title = opts.title || 'KaiLionCrafts 文档';
...
<Application>KaiLionCreator</Application><Company>KaiLionCrafts</Company></Properties>` }
  ];
...
<dc:creator>KaiLionCrafts 锴利匠心</dc:creator>
<cp:lastModifiedBy>KaiLionCreator</cp:lastModifiedBy>
...
export function buildXlsx(sheets, opts = {}) {
  const title = opts.title || 'KaiLionCrafts 表格';
 ...
 */
export function buildPptx(slides, opts = {}) {
  const list = slides.length ? slides : [{ title: opts.title || 'KaiLionCrafts', bullets: [] }];
  const title = opts.title || list[0]?.title || 'KaiLionCrafts 演示文稿';
...
<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideMaster" Target="../slideMasters/slideMaster1.xml"/>
    ...
    { name: 'docProps/app.xml', data: `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties">
<Application>KaiLionCreator</Application><Company>KaiLionCrafts</Company>
...
<a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="KaiLionCrafts">
<a:themeElements>
...
/* ============================================================
   ④ 文本类格式（真实转换，不是模拟）
   ============================================================ */
export function blocksToMarkdown(blocks, meta = {}) {
...
export function blocksToHtml(blocks, meta = {}) {
...
<title>${esc(meta.title || 'KaiLionCrafts 文档')}</title>
<style>
...
<body><div class="wrap">
<header><h1>${esc(meta.title || 'KaiLionCrafts 文档')}</h1>
<div class="meta">KaiLionCrafts 锴利匠心 · 由 KaiLionCreator 生成 · ${new Date().toLocaleDateString('zh-CN')}</div></header>
${body}
<footer>KaiLionCrafts · 阳江市锴利国际贸易有限公司</footer>
</div></body></html>`;
}

/** blocks → { sheets } 供 Excel 使用：优先用表格块，否则按「列: 值」两列 */
export function blocksToSheets(blocks, title) {
...
export function blocksToSlides(blocks, total) {
  const slides = [];
  let cur = null;
  ...
  blocks.forEach(b => {
    ...
    if (!cur) cur = { title: 'KaiLionCrafts', bullets: [], layout: 'cover' };
  ...
  if (!slides.length) slides.push({ title: 'KaiLionCrafts', bullets: [], layout: 'cover' });
...
export function safeFilename(name, ext) {
  const base = String(name || 'kailioncrafts')
    .split('\n')[0]                                   // 只取第一行
    ...
    .replace(/-$/, '') || 'kailioncrafts';
  return `${base}.${ext}`;
}

/** 从一段需求文本里提炼一个适合做文件名的短标题 */
export function deriveTitle(text, fallback = 'KaiLionCrafts') {
...
/* ---------------- 统一入口：blocks → 目标格式字节 ---------------- */
export function renderDocument(blocks, format, opts = {}) {
  ...
  switch (fmt) {
    ...
    case 'html': return { bytes: bytesOf(blocksToHtml(blocks, { title })), mime: 'text/html;charset=utf-8' };
    case 'md':   return { bytes: bytesOf(blocksToMarkdown(blocks, { title })), mime: 'text/markdown;charset=utf-8' };
    case 'txt':  return { bytes: bytesOf(blocksToText(blocks)), mime: 'text/plain;charset=utf-8' };
    case 'csv':  return { bytes: bytesOf('\uFEFF' + blocksToCsv(blocks)), mime: 'text/csv;charset=utf-8' };
    case 'json': return { bytes: bytesOf(JSON.stringify({ title, blocks }, null, 2)), mime: 'application/json' };
    case 'xml':  return { bytes: bytesOf(blocksToXml(blocks, title)), mime: 'application/xml' };
    case 'rtf':  return { bytes: bytesOf(blocksToRtf(blocks, title)), mime: 'application/rtf' };
    case 'epub': return { bytes: buildEpub(blocks, { title }), mime: 'application/epub+zip' };
    case 'pdf':  return { bytes: bytesOf(blocksToHtml(blocks, { title })), mime: 'text/html;charset=utf-8', printHint: true };
    default:     return { bytes: bytesOf(blocksToText(blocks)), mime: 'text/plain;charset=utf-8' };
  }
}

function blocksToXml(blocks, title) {
  return `<?xml version="1.0" encoding="UTF-8"?>
<document title="${xmlEsc(title)}" generator="KaiLionCreator">
${blocks.map(b => {
    if (b.t === 'table') {
