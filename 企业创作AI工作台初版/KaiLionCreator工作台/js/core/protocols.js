/* ============================================================
   KaiLionCrafts Creator · 协议适配层 / Protocol Adapters
   ------------------------------------------------------------
   为什么单独成文件：
     不同厂商的 API 在四个地方不一样 ——
       ① URL 路径   ② 鉴权头   ③ 请求体结构   ④ 响应与流式解析
     providers.js 负责「怎么发」，这里只负责「发什么长什么样」。
...
export function normMessages(messages) {
 ...
 */
export function resolveProtocol(pv) {
...
/** 这套协议需要哪些额外请求头（供网桥与直连共用） */
export function protocolHeaders(pv) {
